...
public class RecipientController {
    ...
    public RecipientController(Inventory theInventory, UserList theUserList, User theUser){
        ...
        realInventory = new Inventory(inventory);
        cart = new Cart();
    ...
    private void requestAidButtonMouseClicked() {                                             
        if(checkAvalibility(cart)) {
            realInventory.subCart(cart);
            realInventory.serializeInventory();
            recipView.dispose();
            Me2UController controller = new Me2UController(realInventory, myUserList, myUser);
    ...
    private void quantityAddButtonMouseClicked() {                                               
        if(this.getRecipView().getQuantityTextField().getText() != null &&
           !itemToCheck.equals("") && itemToCheck != null){
            ...
            if(quant > 0 ){
                if(inventoryStockAvail(quant)) {
                    if(inventory.packageExists(itemToCheck)) {
                        inventory.subPackages(itemToCheck, quant);
                        if(cart.carePackageExists(itemToCheck)) {
                            cart.getPackage(itemToCheck).setQuantity(this.cart.getPackage(itemToCheck).getQuantity() + quant);
                            setCartList();
                        } else {
                            this.cart.addCarePackage(new CarePackage(inventory.getPackage(itemToCheck)));
                            this.cart.getPackage(itemToCheck).setQuantity(quant);
                            setCartList();
                        }
                    } else if(cart.itemExists(itemToCheck)){
                         inventory.updateItemQuantity(itemToCheck, inventory.getItem(itemToCheck).getQuantity() - quant);
                    ...
                    JOptionPane.showMessageDialog(null, "Not enough stuff to fill your request!", "Warnning",
                                    JOptionPane.ERROR_MESSAGE);
                }
            }
    ...
    private boolean inventoryStockAvail(int requestQuant) {
        boolean inStock = false;
        int check;
        if(inventory.packageExists(itemToCheck)) {
            check = inventory.getPackageQuantity(itemToCheck) - requestQuant;
        } else {
            check = inventory.getItem(itemToCheck).getQuantity() - requestQuant;
        }
    ...
    private void setCartList(){
        ...
        ArrayList<CarePackage> packageList = cart.getCarePackages();
        for(int i = 0;i < packageList.size();i++){
            ((DefaultListModel)this.recipView.getCartList().getModel()).addElement(packageList.get(i).getPackageName());
            ((DefaultListModel)this.recipView.getQuantityList().getModel()).addElement(packageList.get(i).getQuantity());
        }
